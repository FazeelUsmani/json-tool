...
## Month-1 cut decisions (2026-05-26)

Codified per the 2026-05-22 review §7 Suggested Improvement #1+#2 — "freeze new features until launch gates closed" + "decide explicitly whether JSONPath/jq/share/email capture are cut." The decisions below override conflicting day-by-day plans in the Week 4 table and the "Share UX" / "Email capture" sections that follow (both **superseded** as noted).

**Shipped in Month 1:**

- **Share links (URL-hash, no server)** — committed 2026-05-26 (`98a3bdf`). Different architecture from the original PLAN.MD spec below: the JSON is compressed into the URL fragment via lz-string, never reaches a server. Result: the "Share UX — explicit privacy contract" section below is **superseded** — no consent modal, no KV/R2 backend, no badge color change needed because the privacy claim ("we never see your data") stays symmetric across paste/drop/sample/URL load/share-link.

**Cut to Month 2:**

- **JSONPath query bar** — **uncut 2026-05-26 (later same day)**: user direction restored JSONPath to M1 scope. Implementation underway. Reasoning for the reversal: smallest of the three cut items (~3-4 h), brand-decoupled (no brand-name copy in the UI surface), real product value (people search "jsonpath online" — measurable SEO + utility). The original cut reasoning (customer-call signal should drive M2 priorities) still holds for jq + email capture; this carve-out is intentional and scoped.
- **jq query bar** — explicit cut, **unchanged**. Same reasoning + larger scope (jq-wasm is ~1 MB + worker host + UI). The "Email capture — specific value exchange" CLI tease can be re-evaluated post-launch.
- **Email capture** — explicit cut. Reasoning: newsletter destination + CLI packaging + Resend/Postmark integration are non-trivial side work that drains attention from the launch gates. Capture mechanism re-prioritizes on first user signal post-launch. The "Email capture — specific value exchange" section below is **superseded**.

**Feature freeze through brand decision:** no new user-facing features ship between 2026-05-26 and brand-decision close-out. Engineering-actionable carry-forwards tracked in `launch-readiness-gate.md` §Feature freeze.
...
- Launch: soft-committed to end of Week 4. **Slipped per 2026-05-22 engineering assessment** (`ENGINEERING_ASSESSMENT.md`): pre-launch correctness work + production gates require ~3 additional days. Realistic target now is W5-Mon/Tue. See `launch-readiness-gate.md` for the ship-blocker punch list separate from the build verification.
...
## Week 5 — Audit response (2026-05-25 onward)

Off-plan correctness + production-gate work surfaced by `ENGINEERING_ASSESSMENT.md` (2026-05-22). Closes ship-blockers, not features. Punch list in `launch-readiness-gate.md`; per-commit detail in `CHANGELOG.md`.

| Day | Shipped / planned |
|---|---|
| Mon (2026-05-25) | **All 5 audit-response slices + UX polish + post-review fixes in one push.** Slice 1: CI workflow (`1b5ff8f`) + Node pin (`c61df8e`) + lint cleanup. Slice 2: path-IDs identity refactor (`c6be7f8` → `520692c`, 6 phases + 12 pathological-key regression tests). Slice 3: `?url=` streaming + security (`a08133c`); native streaming-Blob fix after smoke caught a main-thread accumulation OOM at 200MB (`6781acd`). Slice 3.5: `?url=` pre-fill UX (`be7cf53`) + synchronous `<head>` strip before Plausible's deferred pageview (`ddbfeef`; first attempt had a React-useEffect ordering bug surfaced by post-slice-4 review). Slice 4: dompurify + qs advisories cleared via `package.json` `overrides`, CI audit-bar bumped from `--audit-level=high` to `moderate` (`131807b`). Critical bug fixes: TablePane sort comparator was peeking the row cache by stale `node.path` after Phase 3 — switched to `node.id` (`ddbfeef`). UX gaps closed: paste-to-empty handler (`eb33e40`), click-to-mount type-to-edit (`8c473cb`), RepairDialog DiffEditor unmount race silenced via `setModel(null)` before unmount (`94493de`). Process docs: CONTRIBUTING + CHANGELOG + shadcn vendor README (`3f43b57`); dead-code prune of `stream-json` + `@internal` tags on test-only helpers (`8685d63`). Gate-doc closures + identity-consistency polish entry (`86721f2`). **Hard-blocker status:** 5 of 6 closed (CI, path-IDs, `?url=` streaming, `?url=` security, dompurify); only branch-protection flip on `main` remains, which is a GitHub UI action. |
| Tue | **Launch operational gates:** branch protection flip on `main` (GitHub side); production preview smoke per `launch-readiness-gate.md` Sign-off checklist; `?url=` smoke on a 200MB+ public URL. Brand decision (still pending, user-owned) — unblocks README rewrite + SEO route copy. |
| Wed-Fri | **Remaining soft blockers per gate doc:** README rewrite (~1-2h, brand-blocked), Playwright e2e suite (~4-6h, must pin the two regressions caught in Mon's review per the gate doc), CSP tighten + HSTS + COOP + XFO + Trusted Types (~1-2h, deploy config), SEO route copy (brand-coupled). Customer calls + cold-email overlap data per the Parallel ops track. |
...
- [x] **500MB JSON loads in <60s with progress bar, can navigate spine, search works** *(2026-05-22: telemetry-2250000.json 505MB → 13858ms parse @ 36.4 MB/s, 1.69 GB heap peak / 1.16 GB steady, search-keystroke INP 560ms worst (560ms is borderline "poor" by Chrome thresholds — scoped honestly in B-narrative); spine navigates, search-slow-path with progress bar works; INP regression fix 5c4f3ca brought keystroke cost from 2s to 560ms via inline ASCII case-folding.)*
- [x] **200MB NDJSON loads, paginates by line, expand-line works** *(2026-05-21 → 2026-05-22: detection + line-offset index + LineRow lazy preview + drawer expansion shipped a07266f / 4f19364; in-place caret/Enter expansion 8cd02d7; validated end-to-end 2026-05-22 on telemetry-900000.ndjson 201MB — 899,999 lines, 262 MB heap (1.30× source vs 3.35× for regular JSON — strong B-narrative material), worker idle post-parse.)*
...
- [x] **`jsonrepair` fixes a deliberately-broken LLM JSON sample** *(2026-05-23 W4-Wed, commit 6c03d4e: Repair button in EditorToolbar; JSON.parse-first triage routes to already-valid toast / Monaco DiffEditor dialog / unrepairable toast; 15 tests cover trailing-comma + single-quotes + unquoted-keys + comments + missing-brace + multi-issue + edge cases.)*
- [x] **Table view renders top-level array as sortable table** *(2026-05-23 W4-Wed: read-only third tab in RightPane; column derivation from first 100 sampled rows with null+X→X collapse rule; type-aware sort with null-at-end policy; SORT_DISABLE_THRESHOLD = 10K rows when first row is stub-backed; per-row lazy materialization via WeakMap-cached blob.slice + JSON.parse; activation gated on root.kind === 'array'; 37 tests on columns + sort logic.)*
...
- [x] **`?url=` handler enforces 500MB cap (matching hero), content-type allowlist, 3-redirect max, 30s abort** *(2026-05-23 W4-Wed: DEFAULT_MAX_BYTES bumped 100 → 500 MiB in fetchUrl.ts now that the streaming Tokenizer parser shipped; allowlist + 30s abort were already in place; redirect limit accepts browser default ~20 per the fetchUrl.ts comment, not strictly enforced at 3.)*
...
- [x] **Benchmark methodology page published** (hardware, browser, fixtures, repro) *(2026-05-21 → 2026-05-22: see benchmarks/methodology.md — initial 201MB data, expanded with 505MB browser cold-run + Lighthouse panel scores + INP regression + INP fix; needs to be wired to a public route before launch.)*
...
- [ ] CI green: `tsc && eslint && vitest && playwright smoke` *(2026-05-25: CI workflow shipped — `1b5ff8f` runs `npm ci` + typecheck + lint + test + build + `npm audit --audit-level=moderate` (slice-4 bump) on every push/PR. Tests up to 373/374 passing. **Playwright smoke still missing** — soft blocker per `launch-readiness-gate.md`; once it lands, must pin TablePane sort + `?url=` strip ordering regressions from the post-slice-4 review.)*
